...
function emitKeypressEvents(stream, iface = {}) {
  ...
  stream[KEYPRESS_DECODER] = new import_node_string_decoder.StringDecoder("utf8");
  ...
  function onData(input) {
    if (stream.listenerCount("keypress") > 0) {
      ...
      if (string) {
        (0, import_node_timers.clearTimeout)(timeoutId);
        ...
        for (const character of string) {
          ...
          try {
            ...
            if (length === string.length && character === kEscape2) {
              timeoutId = (0, import_node_timers.setTimeout)(triggerEscape, escapeCodeTimeout);
            }
          } catch (err) {
            stream[ESCAPE_DECODER] = emitKeys(stream);
            stream[ESCAPE_DECODER].next();
